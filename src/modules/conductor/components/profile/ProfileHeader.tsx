...
export function ProfileHeaderConductor({
  title,
  onBack,
}: ProfileHeaderProps) {
  return (
    <header className="relative w-full bg-white px-4 py-5 flex items-center justify-center shadow-sm">
      ...
      <button
        onClick={onBack}
        className="absolute left-4 text-[var(--secondary)] text-2xl hover:scale-110 transition-transform"
      >
        <HiOutlineChevronLeft />
      </button>

      {/* Título */}
      <h1 className="text-lg font-semibold text-[var(--secondary)]" >
        {title}
